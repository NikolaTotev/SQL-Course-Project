...
select distinct * from (select reservations.GuestEGN, Guests.FirstName,Guests.LastName,Guests.ExtraNotes from reservations join Guests on reservations.GuestEGN=Guests.EGN) as roomTypes where FirstName = 'Jane'
select * from (select reservations.roomnumber, rooms.roomtype, reservations.GuestEGN from reservations join rooms on reservations.RoomNumber=rooms.roomNumber) as roomTs join guests on guests.EGN=roomTs.GuestEGN where extranotes='NONE'
select roomNumber, roomType, FirstName, LastName from (select reservations.roomnumber, rooms.roomtype, reservations.GuestEGN, reservations.adults from reservations join rooms on reservations.RoomNumber=rooms.roomNumber) as roomTs join guests on guests.EGN=roomTs.GuestEGN where extranotes='NONE' and roomTs.Adults = 2
